...
namespace STX
{
    public partial class ListFilial_old : Form, IListForm
    {

        private ISelector returnClass;
        private List<Filial> listItems;
        private Filial itemSelecionado;

        public ListFilial_old(ISelector returnSelector = null)
        {
            InitializeComponent();
            this.returnClass = returnSelector;
            if (this.returnClass != null)
            {
                this.Text = Config.APP_NAME + " - Selecione uma " + "Filial";
            }else
            {
                this.Text = Config.APP_NAME + " - " + "Filiais";
            }
        ...
        private void CarregarDados(string filtro = "")
        {
            ...
            cb.Add("nome", filtro, MatchMode.Like);
            listItems = new Filial().Select(cb);
